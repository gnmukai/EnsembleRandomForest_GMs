...
ens_random_forests <- function(df, var, covariates, header=NULL, out.folder=NULL, duplicate=TRUE, n.forests=10L, importance = TRUE, cores=parallel::detectCores()-2, save=TRUE, ntree=1000, mtry=5, var.q = c(0.1,0.5,0.9), mode='bin', weights=NULL){
	...
	# Get all the output from the ensemble
		if(mode=='bin'){
			...
			print("line 159")					 
			pred_ens_trRMSE <- sapply(rf.ens, function(x) ( sqrt(mean((x$preds[x$preds$type=='train',2] - x$preds[x$preds$type=='train',"PRES"])^2)) ))
			pred_ens_teRMSE <- sapply(rf.ens, function(x) ( sqrt(mean((x$preds[x$preds$type=='test',2] - x$preds[x$preds$type=='test',"PRES"])^2)) ))
			pred_ens_trR2 <- sapply(rf.ens, function(x) ( cor(x$preds[x$preds$type=='train',"PRES"], x$preds[x$preds$type=='train',2])^2 ))
			pred_ens_teR2 <- sapply(rf.ens, function(x) ( cor(x$preds[x$preds$type=='test',"PRES"], x$preds[x$preds$type=='test',2])^2 ))									  
